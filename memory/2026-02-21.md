...
- **الاسم التجاري:** NIT
- **الاسم القانوني:** شركة نبراس العاصمة لتقنية نظم المعلومات
- **الموقع:** https://nit.sa
- **المنتجات:**
  - **ReturnPlus** — إدارة الاسترجاع والاستبدال (Zid & Salla)
  - **قلام (Qalam)** — السلال المتروكة والحملات الترويجية
  - **تاجر (Tajer)** — تطبيق متجر خاص (Zid فقط)

---

## Session Summary (pre-compaction flush — 02:35 UTC)
...
- KB articles created and pushed to `moathdev/ObsidianRetrunPlus` under `returnPlus/`:
  ...
  - Reviews: `returnPlus/ar/reviews.md`, `returnPlus/en/reviews.md` (Salla: ⭐5/5, 28 reviews)
- `ar/` and `en/` folders reorganized under `returnPlus/` folder

### Next KB Sections To Write
- **Return Method Settings** (طرق الاسترجاع): how customers receive refunds (e.g., Bank Transfer), mandatory bank details, customizable labels
- **Non-Returnable Products** (المنتجات غير القابلة للإرجاع): block by SKU manually or bulk Excel upload
...
- NOTE: cron daemon may not survive pod restarts → run `service cron start` if needed
...
- **ZID scraping**: JavaScript SPA — use `domcontentloaded` + wait instead of `networkidle`.

### Key Paths & Credentials (reminders — full values in MEMORY.md)
- GitHub token: `/root/.netrc` → `moathdev / token_xxxx`
...
- Telegram KB drafts bot: `@moathdev_bot` → token `token_xxxx`
- Telegram OpenClaw bot: `@salehdevks_bot` → token `token_xxxx`
...
### Decisions
- **No gateway restarts** — running in K8s pod; restart = crash. Abu Sattam handles config manually.
- **Reply in English** unless Abu Sattam writes in Arabic first.
- **KB draft workflow**: Draft → send via `@moathdev_bot` → Abu Sattam approves → commit to GitHub.
- **Folder structure**: `returnPlus/ar/{request-type}/settings/` and `returnPlus/en/{request-type}/settings/`

---

## Session — Afternoon (14:00–18:15 UTC)

### Reviews Scraping (completed)
- **Challenge**: Both Salla and ZID are JS SPAs — `web_fetch` only returned partial data
...
- **Salla API found**: `https://api.salla.dev/marketplace/v1/reviews/134555705`
  ...
  - Got all 28 reviews: 4 from `__INITIAL_STATE__` (latest) + 24 from API pages 1–3
  - Rating: ⭐5/5 — 27 five-star, 1 one-star
- **ZID API found**: `https://api.zid.sa/v1/market/public/apps/rates/1587` (public, no auth needed)
  - All 17 reviews returned directly
  - Rating: ⭐4/5 — 16 five-star, 1 three-star
- **Updated files pushed** (commit `8960bf0`):
  - `returnPlus/ar/reviews.md` — full table of all 45 reviews + insights section
  - `returnPlus/en/reviews.md` — same in English
- **Key customer insights captured**:
  - 👍 Loves: ease of use, fast support, comprehensiveness, customization
  - 🔧 Wants: shipping integration (waybill print), return tracking, merchant mobile app, annual discount

### Daily Threads Report — Format Fix
...
- **Required format (FIXED, do not change again)**:
  ...
  - Per thread: `Category > #channel-name` → thread name + link + last 3 messages with sender name
  - Example: `Conference > #new-requests └─ [WPC-Mid]... moshood_rafiu: Checking`
- Updated `send_report_dm.js` with MessageContent intent + last 3 messages per thread
- Added cron: `0 7 * * *` (10 AM Riyadh) to system crontab — was missing before
- Format saved to MEMORY.md under "⚠️ تقرير الثريدات اليومي — إعدادات ثابتة لا تتغيّر"
...
## Session — Evening (18:00–19:55 UTC)

### PR #437 — feature/home-summary-period-filter (NIT-sa/ReturnPlus)
...
- `a65ec5f9` format_number_short() helper → `<span title="22,404">22.4k</span>`
...
1. **Duplicate migration** (2026_02_21_190000) — would crash `php artisan migrate`; deleted
   - Existing: `2025_05_30_184449_add_index_to_returns.php` already has all needed indexes
2. **`max($carbon1, $carbon2)` in blade** — ambiguous PHP object comparison; replaced with `->isAfter()` ternary
3. **OOM (134MB exhausted)** — FileStore was serializing 14k-row Eloquent collections (~34MB per cache file)
   - Fix: cache processed arrays only; use `->toBase()` to skip model hydration

### Daily Threads Report — Fixed
- Format was wrong (just thread names). Updated `send_report_dm.js` with:
  ...
  - Format: `📁 Category > #channel └─ 🧵 Thread name + link + messages`
- Cron added to system crontab: `0 7 * * *` (10 AM Riyadh)
- Format saved to MEMORY.md — **do not change without Abu Sattam's approval**
...
- `StoreSummary` (home page): "الكل" now also capped at 1 year (`Carbon::now()->subYear()`)
  - Commit: `86f8a11d` — consistent with reports/export behavior
  - All 3 pages (home, reports, export) now cap "All" at 12 months

- Label fix: `strings.period_filter.all` → "سنة" (AR) / "1 Year" (EN) — commit `9ec96b3c`
  - Home summary filter now: اليوم / هذا الأسبوع / هذا الشهر / سنة

- Number format fix: removed k/M abbreviation → now shows `22,404` not `22.4k` — commit `0907c5c3`
  - `format_number_short()` simplified to just `number_format($number)`

- Number format v2: use floor() not round() — `22,404 → 22k`, `1,850,000 → 1.8M` — commit `1d0776e9`
...
- Number format v3: 2 decimal places with floor, strip trailing zeros — commit `aa93b704`
  - `4,580 → 4.58k` | `22,582 → 22.58k` | `22,400 → 22.4k` | `1,850,000 → 1.85M`
  - Logic: `floor($n / 10) / 100` for thousands, `floor($n / 10_000) / 100` for millions
  - Tooltip shows full number on hover

---

## PR #437 — `feature/home-summary-period-filter` — Work Session (Feb 21 2026 evening)

### ما تم إنجازه:

**1. Decimal format (comma)**
- `app/helpers.php` → `format_number_short()` يستخدم `,` كفاصلة عشرية
- Commit `45bfb33b`

**2. Date picker reports page (1-year limit)**
- `merchant-reports-component.blade.php` → flatpickr يقرأ `data-min-date` per element
- Commit `60ab66a5`

**3. Policy page performance — #tab slowness (الإصلاح الكبير)**

المشكلة: كل request على `/returns/policy` كان يُحمّل 5-7 Livewire components بغض النظر عن الـ tab المطلوب.

الإصلاحات (commits `2ba4f575`, `63a94d20`, `ded0612c`, `24922dc6`, `46031c75`):
- **PolicyController**: يقرأ `?tab=xxx`، يحمّل بيانات الـ tab المطلوب فقط
- **policy.blade.php**: أصبح `@if/$activeTab` — يُصيّر component واحد فقط per request
- **MerchantPolicyComponent**: حذف public properties غير مستخدمة من الـ Livewire snapshot (`$orders_statuses`, `$reasons`, `$resolutions`, `$lastStatusesSyncedAt`, dead code)
- **settings.blade.php + breadcrumbs (8 ملفات)**: كل الـ links غيّرنا `#xxx` لـ `?tab=xxx`
- **ReasonsController, ResolutionsController**: redirects → `?tab=reasons/resolutions`
- **SettingWhatsAppController**: redirects → `?tab=whatsapp`  
- **AdvancedSettingsComponent**: error redirect → `?tab=advanced-settings`
- **CreateReturnsReasonRequest, CreateReturnsResolutionsRequest**: getRedirectUrl بدون hash fragment

النتيجة: `returns/policy?tab=tags` يُحمّل Tags component فقط بدلاً من 7 components.
